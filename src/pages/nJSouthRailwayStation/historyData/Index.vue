<template>
  <div class="history-data-page">
    <div class="filter-form">
      <div class="item">
        <el-form :inline="true" :model="filterForm" label-width="0" class="demo-form-inline">
          <el-form-item v-show="false" label="维度切换" label-width="80px">
            <el-select v-model="filterForm.dimension" placeholder="请选择" @change="changeDimension">
              <template v-for="(item, key) in selectOptions">
                <el-option :key="key" :label="item.label" :value="item.value">
                </el-option>
              </template>
            </el-select>
          </el-form-item>
          <el-form-item label="日期" label-width="50px">
            <el-date-picker
              v-model="filterForm.date"
              type="daterange"
              @change="selectDate"
              range-separator="至"
              start-placeholder="开始日期"
              end-placeholder="结束日期"
              :picker-options="pickerOptions">
            </el-date-picker>
          </el-form-item>
        </el-form>
        <!--region 辅助筛选-->
        <el-radio-group v-show="currDimensionCollapse === '1'" size="mini" v-model="inProvince"
                        @change="changeInProvince">
          <el-radio-button :label="false">省外</el-radio-button>
          <el-radio-button :label="true">省内</el-radio-button>
        </el-radio-group>
        <el-radio-group v-show="currDimensionCollapse === '2'" size="mini" v-model="isAge"
                        @change="changeIsAge">
          <el-radio-button :label="true">年龄</el-radio-button>
          <el-radio-button :label="false">性别</el-radio-button>
        </el-radio-group>
        <!--endregion-->
      </div>
    </div>
    <el-tabs v-model="currDimensionCollapse" type="card" @tab-click="handleTabClick">
      <template v-for="(item) in collapseList">
        <el-tab-pane :key="item.value" :label="item.label" :name="item.value">
          <i-table v-if="tableActive" :columns="columns"
                   :tableData="list"
                   :pageSize="pageSize"
                   :v-if="item.value === currDimensionCollapse"
                   :total="list.length"
                   :otherHeight="otherHeight"
                   :options="options"></i-table>
        </el-tab-pane>
      </template>
    </el-tabs>
  </div>
</template>

<script>
// import iTable from '../../../components/iTable/Index'
import iTable from '../../../components/iTable.vue'
import JsonData from '../../../../static/json/data/JyBridge'
import BLL from './index'

export default {
  components: { iTable },
  data () {
    return {
      data: JsonData,
      pageSize: 8,
      columns: [
        {
          prop: 'report_day',
          label: '日期',
          align: 'center'
        },
        {
          prop: 'city_name',
          label: '省份',
          align: 'center'
        },
        {
          prop: 'day_num',
          label: '客流量(人)',
          align: 'center'
        },
        {
          prop: 'rate',
          label: '占比(%)',
          align: 'center'
        }
      ],
      tableActive: true,
      list: [],
      userSourceInProvinceObj: {
        columns: [
          {
            prop: 'report_day',
            label: '日期',
            align: 'center'
          },
          {
            prop: 'city_name',
            label: '地市',
            align: 'center'
          },
          {
            prop: 'day_num',
            label: '客流量(人)',
            align: 'center'
          },
          {
            prop: 'rate',
            label: '占比(%)',
            align: 'center'
          }
        ],
        list: []
      }, // 客流来源分析表格(省内)
      userFlowObj: {
        columns: [
          {
            prop: 'report_day',
            label: '日期',
            align: 'center',
            render (row) {
              return row['report_day'].substring(0, 4) + '-' + row['report_day'].substring(4, 6) + '-' + row['report_day'].substring(6)
            }
          },
          {
            prop: 'countAll',
            label: '总客流量(人)',
            align: 'center'
          },
          {
            prop: 'countInCity',
            label: '本地总客流量(人)',
            align: 'center'
          },
          {
            prop: 'countOutCity',
            label: '省内跨地市客流量(人)',
            align: 'center'
          },
          {
            prop: 'countOutPro',
            label: '省外客流量(人)',
            align: 'center'
          }
        ],
        list: []
      }, // 客流量分析表格
      userSourceOutProvinceObj: {
        columns: [
          {
            prop: 'report_day',
            label: '日期',
            align: 'center'
          },
          {
            prop: 'city_name',
            label: '省外',
            align: 'center'
          },
          {
            prop: 'day_num',
            label: '客流量(人)',
            align: 'center'
          },
          {
            prop: 'rate',
            label: '占比(%)',
            align: 'center'
          }
        ],
        list: []
      }, // 客流来源分析表格(省外)
      userPropertyOfAgeObj: {
        columns: [
          {
            prop: 'report_day',
            label: '日期',
            align: 'center'
          },
          {
            prop: 'cnt_00_22',
            label: '22岁以下(%)',
            align: 'center'
          },
          {
            prop: 'cnt_23_35',
            label: '23-35岁(%)',
            align: 'center'
          },
          {
            prop: 'cnt_36_45',
            label: '36-45岁(%)',
            align: 'center'
          },
          {
            prop: 'cnt_46_55',
            label: '46-55岁(%)',
            align: 'center'
          },
          {
            prop: 'cnt_56_00',
            label: '56岁以上(%)',
            align: 'center'
          }
        ],
        list: []
      }, // 客流属性分析表格（年龄）
      userPropertyOfSexObj: {
        columns: [
          {
            prop: 'report_day',
            label: '日期',
            align: 'center'
          },
          {
            prop: 'woman',
            label: '女性(%)',
            align: 'center'
          },
          {
            prop: 'man',
            label: '男性(%)',
            align: 'center'
          }
        ],
        list: []
      }, // 客流属性分析表格（性别）
      throughDurationObj: {
        columns: [
          {
            prop: 'dd',
            label: '日期',
            align: 'center',
            render (row) {
              try {
                return row['dd'].substring(0, 4) + '-' + row['dd'].substring(4, 6) + '-' + row['dd'].substring(6)
              } catch (e) {
                return row['dd']
              }
            }
          },
          {
            prop: 'spec_road',
            label: '换乘路线',
            align: 'center'
          },
          {
            prop: 'nums',
            label: '人数(人)',
            align: 'center'
          }
        ],
        list: []
      }, // 换乘分析
      selectOptions: [
        {
          value: '0',
          label: '客流量分析'
        }, {
          value: '1',
          label: '客流来源'
        }, {
          value: '2',
          label: '客流属性'
        }, {
          value: '3',
          label: '换乘分析'
        }], // 纬度数据
      filterForm: {
        dimension: '0', // 数据维度
        date: []
      }, // 数据筛选
      pagination: {
        pageIndex: 1,
        pageSize: 20
      }, // 分页参数
      otherHeight: 240, // 表格之外的高度
      options: {
        stripe: true, // 是否为斑马纹 table
        highlightCurrentRow: true, // 是否支持当前行高亮显示
        mutiSelect: false // 是否支持列表项选中功能
      },
      dateInApi: {
        _start: '',
        _end: ''
      }, // 接口需要的日期格式
      inProvince: false, // 是否省内
      isAge: true, // 是否为年龄
      currDimensionCollapse: '0' // 当前维度数据面板
    }
  },
  created () {
    this.BLL = new BLL(this)
    // 默认日期为最近一周
    this.filterForm.date = [new Date(new Date().getTime() - 3600 * 1000 * 24 * 8), new Date(new Date().getTime() - 3600 * 1000 * 24)]
    this.BLL.formatDate()
  },
  mounted () {
    this.BLL.historyFlowContrast()
  },
  computed: {
    // 取数据面板集合
    collapseList () {
      return this.selectOptions
    },
    pickerOptions () {
      var timestamp = parseInt(Date.parse(this.$utils.Date.format(new Date(new Date().getTime() - 24 * 60 * 60 * 1000), 'yyyy-MM-dd')))
      let options = { // eslint-disable-line
        disabledDate (time) {
          // if (!_this.ispickerOption) {
          //   return time.getTime() < 1520611199000
          // }
          return time.getTime() > timestamp
        }
      }
      return options
    }
  },
  methods: {
    // 数据维度切换
    changeDimension (val) {
    },
    // 面板切换，同时切换table表格
    handleTabClick () {
      this.panChange()
    },
    // 面板联动
    panChange () {
      // 客流量分析
      if (this.currDimensionCollapse === '1') {
        // 切换省内省外
        this.BLL.flowProvDay()
        this.BLL.flowCityDay()
        // if (!this.inProvince) {
        //   this.BLL.flowProvDay() // 客流来源-省外
        // } else {
        //   this.BLL.flowCityDay() // 客流来源-省内
        // }
      } else if (this.currDimensionCollapse === '2') {
        this.BLL.areaFlowProperty()
      } else if (this.currDimensionCollapse === '3') {
        this.BLL.getTransferForChartDurationData() // 换乘分析
      } else if (this.currDimensionCollapse === '0') {
        this.BLL.historyFlowContrast() // 总客流量
      }
    },
    // 切换省内省外
    changeInProvince (val) {
      if (!val) {
        this.columns = this.userSourceOutProvinceObj.columns
        this.list = this.userSourceOutProvinceObj.list
      } else {
        this.columns = this.userSourceInProvinceObj.columns
        this.list = this.userSourceInProvinceObj.list
      }
    },
    // 切换用户属性 === 性别/年龄
    changeIsAge (val) {
      if (val) {
        this.columns = this.userPropertyOfAgeObj.columns
        this.list = this.userPropertyOfAgeObj.list
      } else {
        this.columns = this.userPropertyOfSexObj.columns
        this.list = this.userPropertyOfSexObj.list
      }
    },
    // 选择时间
    selectDate () {
      this.BLL.formatDate()
      this.panChange()
    }
  }
}
</script>

<style lang="scss">
.history-data-page {
  background-color: #fff;
  .filter-form {
    height: 60px;
    .item {
      .el-form-item {
        .el-range-separator {
          width: 10%;
        }
      }
      .el-radio-group {
        position: absolute;
        right: 24px;
        z-index: 999;
      }
    }
  }
}
</style>
