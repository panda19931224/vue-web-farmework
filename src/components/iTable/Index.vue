...
<template>
  <div class="table">
    <el-table id="Table" v-loading.iTable="loading"  :data="list" :max-height="height" border
      ...
      <template v-for="(column, index) in columns">
        <el-table-column :key="index" :prop="column.prop" v-if="!column.hidden" :label="column.label" :align="column.align"
                         :width="column.width"
...
export default {
  props: {
    ...
    operates: {
      type: Object,
      default: null// width:按钮列宽，fixed：是否固定（left,right）,按钮集合 === label: 文本，type :类型（primary / success / warning / danger / info / text），show：是否显示，icon：按钮图标，plain：是否朴素按钮，disabled：是否禁用，method：回调方法
    },
    total: {
      type: Number,
      default: 0
