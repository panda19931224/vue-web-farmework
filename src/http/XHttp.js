...
function requestUrl (url) {
  ...
  if (process.env.NODE_ENV !== 'development') {
    if (url.startsWith('iTransport')) {
      // 测试环境
      if (process.env.NODE_ENV === 'testing') {
        // 测试环境，访问测试接口
        url = url.replace('iTransport', config.build.offlineApiUrl)
      } else {
        url = url.replace('iTransport', config.build.onlineApiUrl)
      }
